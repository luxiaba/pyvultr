...
from pyvultr.v2 import ObjectStorage, ObjectStorageClusterItem, ObjectStorageS3Credential
...
class TestObjectStorage(BaseTestV2):
    def test_list(self):
        ...
        with self._get("response/object_storages") as mock:
            ...
            excepted_result = ObjectStorage.from_dict(_excepted_result)

            _real_result = self.api_v2.object_storage.list()
            real_result: ObjectStorage = _real_result.first()
    ...
    def test_create(self):
        """Test list object storages."""
        with self._post("response/object_storage", expected_returned=ObjectStorage, status_code=202) as mock:
            excepted_result = mock.python_body

            cluster_id = str(uuid.uuid4())
            label = "test_label"
            real_result: ObjectStorage = self.api_v2.object_storage.create(cluster_id=cluster_id, label=label)
    ...
    def test_get(self):
        ...
        with self._get("response/object_storage", expected_returned=ObjectStorage) as mock:
            excepted_result = mock.python_body

            object_storage_id = str(uuid.uuid4())
            real_result: ObjectStorage = self.api_v2.object_storage.get(object_storage_id)
